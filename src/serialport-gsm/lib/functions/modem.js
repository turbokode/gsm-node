...
module.exports = function (SerialPort) {
  ...
  modem.emitDelayedNewIncomingMessageByReference = (referenceNumber) => {
    const resultData =
      modem.incomingSMSIndicationTimeoutsByReference.get(referenceNumber);
  ...
  modem.readSMSById = function (id, callback) {
    ...
    item.logic = (newpart) => {
      ...
      if (newpart.includes("+CMGR:")) {
        ...
        resultData.data[resultData.data.length - 1] = self.processMessage(
          newMessage,
          messageIndex,
          messageStatus
        );
        ...
        newpart.toString().length >= 1 &&
        resultData.data[resultData.data.length - 1] &&
        isNaN(resultData.data[resultData.data.length - 1].msgStatus)
      ) {
        ...
        resultData.data[resultData.data.length - 1] = {
          ...
          index: parseInt(
            resultData.data[resultData.data.length - 1].index,
            10
          ),
          msgStatus:
            resultData.data[resultData.data.length - 1].msgStatus || null,
          dateTimeSent:
            resultData.data[resultData.data.length - 1].dateTimeSent || null,
          header: {
            encoding: null,
            smsc: null,
            smscType: null,
    ...
    parts.forEach((part) => {
      ...
      newparts.forEach((newpart) => {
        ...
        if (modem.incomingSMSIndication && newpart.startsWith("+CMTI:")) {
          // New Message Indicator with SIM Card ID, After Receiving Read The Message From the SIM Card
          ...
          modem.readSMSById(id, (result, error) => {
            if (!error) {
              if (
                ...
                let messageData =
                  modem.incomingSMSIndicationTimeoutsByReference.get(
                    referenceNumber
                  );
                if (messageData) {
                ...
                modem.logger.debug(
                  `Incoming multipart message delayed processing (reference=${referenceNumber}): ${messageData.data.length}/${messageData.data[0].udh.parts} received`
                );
                // still not all parts were received, wait or timeout
                if (messageData.data.length !== result.data[0].udh.parts) {
                  messageData.emitDelay = setTimeout(
                    (referenceNumber) =>
                      modem.emitDelayedNewIncomingMessageByReference(
                        referenceNumber
                      ),
                    modem.incomingSMSIndicationDelay,
                    referenceNumber
                  );
                  modem.incomingSMSIndicationTimeoutsByReference.set(
                    referenceNumber,
                    messageData
                  );
                  return;
                }
                modem.incomingSMSIndicationTimeoutsByReference.delete(
                  referenceNumber
                );
                result = messageData;
              }
...
Helper.decode7Bit = function (text, inLen, alignBits) {
  ...
  while (true) {
    ...
    if (inLen === undefined) {
      ...
      if (dataPos == data.length && bufLen == 7 && !buf) break;
    } else {
      if (inDone >= inLen) break;
    }
  }
